...
export async function POST(req: NextRequest) {
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            'You are a helpful assistant. Reply ONLY with minified JSON in this shape: {"reply": string, "sentiment": "positive"|"neutral"|"negative", "keywords": string[], "actions"?: [{"type": string, "value"?: string}]}. No extra text.',
        },
    ...
    await prisma.chatRecord.create({
      data: { userInput, aiJson, model: completion.model },
    });
